    ...
    suspend fun detectObjects(bitmap: Bitmap): List<DetectedObject> =
        suspendCancellableCoroutine { cont ->
            ...
            objectDetector.process(image)
                .addOnSuccessListener { results: List<DetectedObject> -> cont.resume(results) }
                .addOnFailureListener { e: Exception -> cont.resumeWithException(e) }
        }
    ...
    suspend fun labelImage(bitmap: Bitmap): List<ImageLabel> =
        suspendCancellableCoroutine { cont ->
            ...
            imageLabeler.process(image)
                .addOnSuccessListener { labels: List<ImageLabel> -> cont.resume(labels) }
                .addOnFailureListener { e: Exception -> cont.resumeWithException(e) }
        }
    ...
    suspend fun translateText(text: String, targetLanguage: String = "en"): String {
        ...
        suspendCancellableCoroutine<Unit> { cont ->
            translator.downloadModelIfNeeded()
                .addOnSuccessListener { _: Void? -> cont.resume(Unit) }
                .addOnFailureListener { e: Exception -> cont.resumeWithException(e) }
        }

        return suspendCancellableCoroutine { cont ->
            translator.translate(text)
                .addOnSuccessListener { translatedText: String -> cont.resume(translatedText) }
                .addOnFailureListener { e: Exception -> cont.resumeWithException(e) }
        }
    }
}
