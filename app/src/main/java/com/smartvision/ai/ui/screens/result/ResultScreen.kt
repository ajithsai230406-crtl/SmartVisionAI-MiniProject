...
import com.smartvision.ai.ui.theme.SVColorScheme
import com.smartvision.ai.ui.theme.moduleAccent
                    ...
                    Box {
                        ...
                        DropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }) {
                            listOf(
                                ...
                                "Re-scan" to { onRescan(); showMenu = false },
                                "Open in Browser" to {
                                    val qr = s.scanResult
                                    if (qr is ScanResult.QrCodeResult && qr.type == QrType.URL) {
                                        context.startActivity(
                                            Intent(Intent.ACTION_VIEW, Uri.parse(qr.rawValue))
                                ...
                                DropdownMenuItem(text = { Text(label) }, onClick = { action(); showMenu = false })
                            }
                        }
                    }
                }
        ...
        is ScanResult.ObjectDetectionResult -> {
            ...
            result.objects.forEach { obj ->
                Row(
                    ...
                    Text(obj.label, style = MaterialTheme.typography.titleMedium, color = c.onSurface, fontWeight = FontWeight.SemiBold)
                    ConfidenceBadge(obj.confidence)
                }
            }
        }
        is ScanResult.OcrResult -> {
            Text("Extracted Text", style = MaterialTheme.typography.titleMedium, color = c.onSurface, fontWeight = FontWeight.Bold)
            Box(
                Modifier
                    .fillMaxWidth()
                ...
                SelectionContainer {
                    Text(result.rawText, style = MaterialTheme.typography.bodyMedium, color = c.onSurface)
                }
            }
        }
            ...
            result.categories.forEach { cat ->
                Column(
                    ...
                    Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
                        Text(cat.label, style = MaterialTheme.typography.titleSmall, color = c.onSurface, fontWeight = FontWeight.SemiBold)
    ...
    Box(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                ...
                Text("AI Explanation", style = MaterialTheme.typography.titleSmall, color = accent, fontWeight = FontWeight.Bold)
            }
