...
fun ObjectDetectionScreen(
    ...
    val boxColors = listOf(SVColors.cyan, SVColors.amber, SVColors.green, SVColors.blue, SVColors.purple, SVColors.orange)
    ...
    Scaffold(
        ...
        Column(
            ...
            Box(
                ...
                if (path != null) {
                    AsyncImage(path, null, contentScale = ContentScale.Crop, modifier = Modifier.fillMaxSize())
                    if (result is ScanResult.ObjectDetectionResult) {
                        val objs = (result as ScanResult.ObjectDetectionResult).objects
                        androidx.compose.foundation.Canvas(Modifier.fillMaxSize()) {
                            objs.forEachIndexed { i, obj ->
                                val col = boxColors[i % boxColors.size]
                                val b = obj.boundingBox
                                val l = b.left * size.width; val t = b.top * size.height
                                val w = (b.right - b.left) * size.width; val h = (b.bottom - b.top) * size.height
                    ...
                    Column(Modifier.fillMaxSize(), Arrangement.Center, Alignment.CenterHorizontally) {
                        ...
                        Text("Capture an image to detect objects", style = MaterialTheme.typography.bodyMedium, color = c.subtext)
                    }
                }
                if (loading) Box(Modifier.fillMaxSize().background(Color.Black.copy(.5f)), Alignment.Center) {
                        ...
                        Row(
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                                Box(
                                    ...
                                    Box(Modifier.fillMaxHeight().fillMaxWidth(obj.confidence).background(col, CircleShape))
                                }
