    ...
    Scaffold(
        topBar = { SmartVisionTopBar("Text Scanner (OCR)", onBack = onBack) {
            if (s.selectedText.isNotEmpty() || s.fullText.isNotEmpty()) {
                IconButton(onClick = { vm.copy() }) { Icon(Icons.Rounded.ContentCopy, null, tint = c.primary) }
                IconButton(onClick = { vm.speak() }) { Icon(Icons.Rounded.VolumeUp, null, tint = c.primary) }
            }
        }},
        ...
        Column(Modifier.fillMaxSize().padding(pad), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Column(Modifier.fillMaxWidth().weight(.45f).padding(horizontal = 14.dp),
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    val actions = listOf(
                        OcrAction(Icons.Rounded.ContentCopy, "Copy") { vm.copy() },
                        OcrAction(Icons.Rounded.Translate, "Translate") {
                            val t = s.selectedText.ifEmpty { s.fullText }
                            onTranslate?.invoke(t)
                        },
                        OcrAction(Icons.Rounded.VolumeUp, "Speak") { vm.speak() }
                    )
                    for (actionItem in actions) {
                        FilterChip(
                            selected = false,
                            onClick = actionItem.onClick,
                            label = { Text(actionItem.label, style = MaterialTheme.typography.labelLarge) },
                            leadingIcon = { Icon(actionItem.icon, null, modifier = Modifier.size(15.dp)) },
