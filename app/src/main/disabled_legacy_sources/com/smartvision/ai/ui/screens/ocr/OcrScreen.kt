...
// ─────────────────────────────────────────────────────────────────────────────
// OCR SCREEN
// CHANGE: after translation of OCR text, show original scanned text beneath
//         translated result so user can compare in their own language.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    Scaffold(
        ...
        Column(
            ...
            Box(
                ...
                // Text block highlights + selection overlay
                Canvas(modifier = Modifier.fillMaxSize()) {
                    ...
                    if (isDragging || uiState.selectedText.isNotEmpty()) {
                        ...
                        listOf(
                            ...
                            drawCircle(Color(0xFF00E5FF), 6.dp.toPx(), corner)
                        }
                    }
                }
            ...
            Column(
                ...
                Row(
                    ...
                    Text(
                        text       = if (uiState.selectedText.isNotEmpty()) "Selected Text" else "Extracted Text",
                ...
                if (displayText.isEmpty() && !uiState.isLoading) {
                    ...
                    Box(
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                            // Scanned / selected text
                            androidx.compose.foundation.text.selection.SelectionContainer {
                                Text(displayText, style = MaterialTheme.typography.bodyMedium, color = colors.onSurface)
                            }

                            // ── Translated result WITH original shown below ──
                            // (satisfies the requirement: show scanned-language text
                            //  below translation so user understands it in their language)
                            if (uiState.translatedText.isNotEmpty()) {
                                HorizontalDivider(color = SmartVisionColors.textScanner.copy(0.25f))
                                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                                    ...
                                    // Original scanned text label underneath
